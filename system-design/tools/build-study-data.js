...
/* =========================================================
   ...
   Parses every NN-slug.html chapter and emits two globals:
     ...
     window.SD_INDEX — the Ctrl+K search index
                       [{ n, f, ti, lv, e: [{ t, a, k }] }]
                       k is 'section' | 'demo' | 'topic' | 'fact'
     window.SD_CARDS — the flashcard deck, built from the quizzes
                       [{ n, ti, f, lv, q, o[], a, w }]
...
window.SD_INDEX = ${JSON.stringify(index)};
window.SD_CARDS = ${JSON.stringify(cards)};
`;
