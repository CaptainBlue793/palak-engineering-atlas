...
/* =========================================================
   ...
   <script> is captured into window.DSA_PAGES. A tiny hash router
...
const OUT_FILE = path.join(OUT_DIR, 'dsa-course.html');
...
appJs = patch(appJs, 'assets/app.js',
  ...
  `function go(r) { if (r) { close(); DSA.go(r.f, r.a); } }`);
appJs = patch(appJs, 'assets/app.js',
  `const here = location.pathname.split('/').pop();`,
  `const here = window.DSA_ROUTE || location.pathname.split('/').pop();`);

pages['index.html'].scripts = pages['index.html'].scripts.map((s) => patch(
  s, 'index.html',
  `n.addEventListener('click', () => location.href = ch.file);`,
  `n.addEventListener('click', () => DSA.go(ch.file));`,
  /* optional */ true));
if (!pages['index.html'].scripts.some((s) => s.includes('DSA.go(ch.file)'))) {
...
<meta name="description" content="Palak Deb Patra's DSA Playlist — 56 interactive chapters: every data structure and algorithm, with animated diagrams, visualizers, C++ implementations and quizzes. Single-file offline edition.">
...
<script>window.DSA_PAGES = ${json(pages)};</script>
<script>${studyData}</script>
<script>window.__DSA_BOOT = function () {\n${appJs}\n};</script>
