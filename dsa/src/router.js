...
(function () {
  ...
  function render() {
    ...
    window.DSA_ROUTE = r.file;
    document.title = page.title;
    document.getElementById('dsa-page-style').textContent = page.styles || '';
    ...
    try {
      ...
      console.error('[dsa] runtime failed on ' + r.file, e);
    }
    if (window.DSA) window.DSA.go = go;

    for (var i = 0; i < page.scripts.length; i++) {
      try {
        new Function(page.scripts[i])();
      } catch (e) {
        console.error('[dsa] page script ' + (i + 1) + ' failed on ' + r.file, e);
      }
    }
