...
/* =========================================================
   ...
   Parses every NN-slug.html chapter and emits two globals:
     ...
     window.DSA_INDEX — the Ctrl+K search index
                       [{ n, f, ti, lv, e: [{ t, a, k }] }]
                       k is 'section' | 'demo' | 'topic' | 'fact'
     window.DSA_CARDS — the flashcard deck, built from the quizzes
                       [{ n, ti, f, lv, q, o[], a, w }]
...
window.DSA_INDEX = ${JSON.stringify(index)};
window.DSA_CARDS = ${JSON.stringify(cards)};
`;
